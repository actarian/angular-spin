...
import { ControlBase, ControlBaseOptions, FormService } from '../forms';
import { Page, PageResolver, PageResolverService, Pages } from '../pages';
...
export class EditorComponent extends DisposableComponent implements AfterViewInit {
	...
	get componentName(): string {
		if (this._page) {
			const component = this.pages[this._page.component];
			if (component) {
	...
	onKeydown(e: KeyboardEvent) {
		if (e.key === 'e' && e.ctrlKey) {
			this.editing = environment.editor && !this.editing;
